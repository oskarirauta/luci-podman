...
module("luci.controller.podman",package.seeall)

function index()
	local e = entry({"admin", "podman"}, firstchild(), "Podman", 40)
	e.dependent = false
	e.acl_depends = { "luci-app-podman" }

	entry({"admin", "podman", "overview"},form("podman/overview"),_("Overview"), 1).leaf=true
	...
	if (require "luci.model.podman").new():_ping().code ~= 200 then
		return
	end

	entry({"admin", "podman", "containers"}, form("podman/containers"), _("Containers"), 2).leaf=true
	entry({"admin", "podman", "images"}, form("podman/images"), _("Images"), 3).leaf=true
	entry({"admin", "podman", "networks"}, form("podman/networks"), _("Networks"), 4).leaf=true
	entry({"admin", "podman", "volumes"}, form("podman/volumes"), _("Volumes"), 5).leaf=true
	entry({"admin", "podman", "events"}, call("action_events"), _("Events"), 6)

	entry({"admin", "podman", "newcontainer"}, form("podman/newcontainer")).leaf=true
	entry({"admin", "podman", "newnetwork"}, form("podman/newnetwork")).leaf=true
	entry({"admin", "podman", "container"}, form("podman/container")).leaf=true

	entry({"admin", "podman", "container_stats"}, call("action_get_container_stats")).leaf=true
	entry({"admin", "podman", "container_get_archive"}, call("download_archive")).leaf=true
	entry({"admin", "podman", "container_put_archive"}, call("upload_archive")).leaf=true
	entry({"admin", "podman", "images_save"}, call("save_images")).leaf=true
	entry({"admin", "podman", "images_load"}, call("load_images")).leaf=true
	entry({"admin", "podman", "images_import"}, call("import_images")).leaf=true
	entry({"admin", "podman", "images_get_tags"}, call("get_image_tags")).leaf=true
	entry({"admin", "podman", "images_tag"}, call("tag_image")).leaf=true
	entry({"admin", "podman", "images_untag"}, call("untag_image")).leaf=true
	entry({"admin", "podman", "confirm"}, call("action_confirm")).leaf=true
...
function action_events()
	...
	luci.template.render("podman/logs", {self={syslog = logs, title="Events"}})
end
...
function save_images(container_id)
	...
	podman:write_status("Images: saving" .. " " .. container_id .. "...")
...
function load_images()
	...
	podman:write_status("Images: loading...")
	...
	if res.code == 200 and msg and msg:match("Loaded image ID") then
		...
		podman:append_status("code:" .. res.code.." ".. msg)
...
function import_images()
	...
	podman:write_status("Images: importing".. " ".. itag .."...\n")
	...
	}, podman.import_image_show_status_cb)
