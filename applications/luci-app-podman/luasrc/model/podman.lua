...
local podman = require "luci.podman"
local fs = require "nixio.fs"

local _podman = {}
_podman.options = {
	debug = true,
	debug_path = "/tmp/.podman_debug",
	status_path = "/tmp/.podman_status"
}

--pull image and return iamge id
local update_image = function(self, image_name)
	local json_stringify = luci.jsonc and luci.jsonc.stringify
	_podman:append_status("Images: " .. "pulling" .. " " .. image_name .. "...\n")
	local res = self.images:create({query = {fromImage=image_name}}, _podman.pull_image_show_status_cb)
...
_podman.clear_empty_tables = function ( t )
	local k, v
	...
	if next(t) == nil then
		...
		for k, v in pairs(t) do
			if type(v) == 'table' then
				t[k] = _podman.clear_empty_tables(v)
			end
		end
	end
...
local get_config = function(container_config, image_config)
	...
	config.Pod = map_subtract(config.Pod, image_config.Pod)
	...
	create_body = _podman.clear_empty_tables(create_body) or {}
	extra_network = _podman.clear_empty_tables(extra_network) or {}

	return create_body, extra_network
end

local upgrade = function(self, request)
	_podman:clear_status()
	...
	_podman:append_status("Container: " .. "Stop" .. " " .. container_name .. "...")
	...
	_podman:append_status("Container: rename" .. " " .. container_name .. " to ".. container_name .. "_old ...")
	...
	_podman:append_status("Container: Create" .. " " .. container_name .. "...")
	create_body = _podman.clear_empty_tables(create_body)
	...
	for k, v in pairs(extra_network) do
		_podman:append_status("Networks: Connect" .. " " .. container_name .. "...")
...
_podman.new = function()

	local _new = podman.new(_podman.options)
...
_podman.append_status=function(self,val)
	if not val then
		return
	end
	local file_podman_action_status=io.open(self.options.status_path, "a+")
	file_podman_action_status:write(val)
	file_podman_action_status:close()
end

_podman.write_status=function(self,val)
	if not val then
		return
	end
	local file_podman_action_status=io.open(self.options.status_path, "w+")
	file_podman_action_status:write(val)
	file_podman_action_status:close()
end

_podman.read_status=function(self)
	return fs.readfile(self.options.status_path)
end

_podman.clear_status=function(self)
...
_podman.import_image_show_status_cb = function(res, source)
	return status_cb(res, source, function(chunk)
		local json_parse = luci.jsonc.parse
		local step = json_parse(chunk)
		if type(step) == "table" then
