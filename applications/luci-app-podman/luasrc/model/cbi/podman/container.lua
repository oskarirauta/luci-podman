...
local m, s, o
local pods, images, networks, container_info, res
...
local start_stop_remove = function(m, cmd)
	...
	podman:clear_status()
	podman:append_status("Containers: " .. cmd .. " " .. container_id .. "...")
	...
	if res and res.code >= 300 then
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/podman/container/" .. container_id))
	else
		podman:clear_status()
...
local pod_start_stop_remove = function(m, cmd)
	local res

	podman:clear_status()
	podman:append_status("Pods: " .. cmd .. " " .. pod_id .. "...")

	if cmd ~= "upgrade" then
		res = dk.pods[cmd](dk, {id = pod_id})
	else
		res = dk.pods_upgrade(dk, {id = pod_id})
	end

	if res and res.code >= 300 then
		podman:append_status("code:" .. res.code .. " " .. (res.body.message and res.body.message or res.message))
		luci.http.redirect(luci.dispatcher.build_url("admin/podman/pod/" .. pod_id))
	else
		podman:clear_status()
		if cmd ~= "remove" and cmd ~= "upgrade" then
			luci.http.redirect(luci.dispatcher.build_url("admin/podman/pod/" .. pod_id))
		else
			luci.http.redirect(luci.dispatcher.build_url("admin/podman/pods"))
		end
	end
end

m=SimpleForm("podman", container_info.Name, translate("podman Container") )
m.redirect = luci.dispatcher.build_url("admin/podman/containers")
...
o.forcewrite = true
o.write = function(self, section)
	luci.http.redirect(luci.dispatcher.build_url("admin/podman/newcontainer/duplicate/"..container_id))
end
...
if action == "info" then
	...
	local _image = "-"
	if container_info.Image and container_info.Image ~= "" then
		_image = container_info.Image:sub(1, 12)
		for i, v in ipairs(images) do
			if v.Id == container_info.Image then
				_image = v.Names[1] and ( v.Names[1] .. " (" .. v.Id:sub(1, 12) .. ")") or v.Id:sub(1, 12)
				break
			end
		end
	end
	...
	local _pod = "-"
	if container_info.Pod and container_info.Pod ~= "" then
		_pod = container_info.Pod:sub(1, 12)
		for i, v in ipairs(pods) do
			if v.Id == container_info.Pod then
				_pod= v.Name ~= "" and ( v.Name .. " (" .. v.Id:sub(1, 12) .. ")") or v.Id:sub(1, 12)
				break
			end
		end
	end
	...
	if container_info.State.Status == "running" then
		table_info["007start"] = {
			_key = translate("Started"),
			_value = container_info.State and container_info.State.StartedAt or "-"
		}
	else
		table_info["007start"] = {
			_key = translate("Finished"),
			_value = container_info.State and container_info.State.FinishedAt or "-"
		}
	end

	table_info["008healthy"] = {
		_key = translate("Healthcheck status"),
		_value = container_info.State and container_info.State.Healthcheck and container_info.State.Healthcheck.Status or "-"
	}
	table_info["009restart"] = {
	...
	for _, v in ipairs (networks) do
		if v.Name then
			local net_driver = v.Plugins[1].Network.type and v.Plugins[1].Network.type ~= "" and v.Plugins[1].Network.type or ""
			local net_name = v.Name and v.Name or translate("Unknown")
			local net_delimeter = net_driver ~= "" and net_name ~= "" and " | " or ""
			list_networks[v.Name] = net_name .. net_delimeter .. net_driver
		end
	end

	if type(info_networks)== "table" then
		for k,v in pairs(info_networks) do
			table_info["017network"..k] = {
	...
	o.write = function(self, section, value)
		...
		if section == "001name" then
			podman:append_status("Containers: rename " .. container_id .. "...")
		...
		elseif section == "009restart" then
			podman:append_status("Containers: update " .. container_id .. "...")
		...
		elseif table_info[section]._key == translate("Network") then
			...
			podman:append_status("Network: disconnect " .. leave_network .. container_id .. "...")
		...
		elseif section == "18connect" then
			...
			podman:append_status("Network: connect " .. connect_network .. container_id .. "...")
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/podman/container/"..container_id.."/info"))
	end
elseif action == "resources" then
	...
	m.handle = function(self, state, data)
		if state == FORM_VALID then
			...
			podman:write_status("Containers: update " .. container_id .. "...")
			...
			luci.http.redirect(luci.dispatcher.build_url("admin/podman/container/"..container_id.."/resources"))
		end
	end

elseif action == "file" then
	s = m:section(SimpleSection)
	s.template = "podman/container_file"
...
elseif action == "console" then
	m.submit = false
	m.reset  = false
	local cmd_podman = luci.util.exec("command -v podman"):match("^.+podman") or nil
	local cmd_ttyd = luci.util.exec("command -v ttyd"):match("^.+ttyd") or nil

	if cmd_podman and cmd_ttyd and container_info.State.Status == "running" then
		local cmd = "/bin/sh"
		local uid
		...
		o.write = function(self, section)
			...
			if not cmd_podman or not cmd_ttyd or cmd_podman:match("^%s+$") or cmd_ttyd:match("^%s+$")then
				return
			end
			...
			local hosts = "/run/podman/podman.sock"
			...
			local start_cmd = string.format('%s -d 2 --once -p 7682 %s -H "unix://%s" exec -it %s %s %s&', cmd_ttyd, cmd_podman, hosts, uid, container_id, cmd)

			os.execute(start_cmd)

			o = s:option(DummyValue, "console")
			o.container_id = container_id
			o.template = "podman/container_console"
		end
	end
elseif action == "stats" then
	...
	if type(container_top) == "table" then
		...
		s.template = "podman/container_stats"
		table_stats = {
			cpu={
